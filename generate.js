// Template-based addendum generator
// Uses the original Прил_14Каржас-Моджо.docx as base template
// Replaces text in XML while preserving ALL original formatting

const JSZip = require('jszip');

// Original template embedded as base64
const TEMPLATE_B64 = '';

function escapeXml(str) {
  return (str || '').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;');
}

async function fillTemplate(data) {
  // Load template
  const templateBuf = Buffer.from(TEMPLATE_B64, 'base64');
  const zip = await JSZip.loadAsync(templateBuf);
  
  // Get document XML
  let xml = await zip.file('word/document.xml').async('string');
  
  const num = data.num || '14';
  const contract = data.contract || 'KAR-MOJ-USD 01/2025';
  const dateContract = data.date_contract || '02.07.2025';
  const date = data.date || '23.03.2026';
  
  // Helper: replace text content between w:t tags
  // Handles split runs by joining them first
  
  function replaceText(oldText, newText) {
    // Simple replacement of text within w:t tags
    const escaped_old = oldText.replace(/[.*+?^${}()|[\]\]/g, '\\$&');
    const escaped_new = escapeXml(newText);
    xml = xml.replace(new RegExp(escaped_old.replace(/\\/g,'\\'), 'g'), escaped_new);
  }
  
  // Replace addendum number
  xml = xml.replace(/>14</g, `>${escapeXml(num)}<`);
  
  // Replace date (the date appears split across multiple runs: "2", "3", ".", "03", ".20", "26")
  // We need to replace the actual date values
  const oldDate = '23.03.2026';
  const newDate = date;
  if (oldDate !== newDate) {
    // Replace in a smart way - find the pattern in context
    xml = xml.replace(/: 2<\/w:t>[\s\S]*?<w:t[^>]*>3<\/w:t>[\s\S]*?<w:t[^>]*>\.<\/w:t>[\s\S]*?<w:t[^>]*>03<\/w:t>[\s\S]*?<w:t[^>]*>\.20<\/w:t>[\s\S]*?<w:t[^>]*>26</g, 
      (match) => match.replace(': 2', `: ${date.slice(0,1)}`).replace(/>3</, `>${date.slice(1,2)}<`));
    // Simpler: just replace the specific split pattern
    const d = newDate.split('.');
    if (d.length === 3) {
      const dd = d[0], mm = d[1], yyyy = d[2];
      xml = xml.replace('>: 2<', `>: ${dd.slice(0,1)}<`);
      xml = xml.replace(/>3<\/w:t>(<\/w:r><w:r[^>]*><w:rPr[^<]*<\/w:rPr>)?<w:t[^>]*>\.</, `>${dd.slice(1)}<\/w:t><w:t xml:space="preserve">.`);
    }
  }
  
  // Content replacements - direct text substitution
  const replacements = [
    // Goods
    ['Crushed Cocoa Bean Shell', data.goods_en || 'Crushed Cocoa Bean Shell'],
    ['Country of Origin: Nigeria', 'Country of Origin: ' + (data.origin_en || 'Nigeria')],
    ['Какао-велла (шелуха) дробленая', data.goods_ru || 'Какао-велла (шелуха) дробленая'],
    ['Страна происхождения: ', 'Страна происхождения: '],
    ['Нигерия', data.origin_ru || 'Нигерия'],
    // Quantity  
    ['25 tons nett,', (data.qty_en || '25 tons nett') + ','],
    ['25 тонн нетто', data.qty_ru || '25 тонн нетто'],
    // Price
    ['1140 USD/MT', data.price_en || '1140 USD/MT'],
    ['Saint-Petersburg, Russia', data.incoterm || 'Saint-Petersburg, Russia'],
    ['1140 USD/т', data.price_ru || '1140 USD/т'],
    ['Санкт-Петербург, Россия', data.incoterm_ru || 'Санкт-Петербург, Россия'],
    // Total
    ['28500USD', escapeXml(data.total_en || "28'500 USD")],
    ['28500 USD', escapeXml(data.total_ru || "28'500 USD")],
    // Payment
    ['Payment within 4 weeks after invoice date', data.payment_en || 'Payment within 4 weeks after invoice date'],
    ['Оплата в течение 4 недель с момента выставления инвойса ', (data.payment_ru || 'Оплата в течение 4 недель с момента выставления инвойса') + ' '],
    // Packaging
    ['25 kg  net bags', data.pack_en || '25 kg net bags'],
    ['Мешки по 25 кг нетто   ', (data.pack_ru || 'Мешки по 25 кг нетто') + '   '],
    // Delivery
    ['Delivery  in April 2026', data.delivery_en_1 || 'Delivery in April 2026'],
    ['CFR Saint-Petersburg, Russia', data.delivery_en_2 || 'CFR Saint-Petersburg, Russia'],
    ['Поставка в апреле 2026 г. ', (data.delivery_ru_1 || 'Поставка в апреле 2026 г.') + ' '],
    ['CFR Санкт-Петербург, Россия', data.delivery_ru_2 || 'CFR Санкт-Петербург, Россия'],
  ];
  
  for (const [from, to] of replacements) {
    xml = xml.split(escapeXml(from)).join(escapeXml(to));
    xml = xml.split(from).join(escapeXml(to));
  }
  
  // Update zip with new XML
  zip.file('word/document.xml', xml);
  
  return await zip.generateAsync({ type: 'nodebuffer', compression: 'DEFLATE' });
}

exports.handler = async (event) => {
  if (event.httpMethod === 'OPTIONS') {
    return { statusCode: 200, headers: { 'Access-Control-Allow-Origin': '*', 'Access-Control-Allow-Headers': 'Content-Type' } };
  }
  if (event.httpMethod !== 'POST') {
    return { statusCode: 405, body: 'Method not allowed' };
  }
  try {
    const data = JSON.parse(event.body);
    const buf = await fillTemplate(data);
    const num = data.num || 'X';
    const contract = (data.contract || '').replace(/[^a-zA-Z0-9]/g,'_').slice(0,20);
    return {
      statusCode: 200,
      headers: {
        'Content-Type': 'application/vnd.openxmlformats-officedocument.wordprocessingml.document',
        'Content-Disposition': `attachment; filename="Addendum_N${num}_${contract}.docx"`,
        'Access-Control-Allow-Origin': '*',
      },
      body: buf.toString('base64'),
      isBase64Encoded: true,
    };
  } catch (err) {
    console.error(err);
    return { statusCode: 500, body: JSON.stringify({ error: err.message }) };
  }
};
