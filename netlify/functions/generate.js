...
function rep(x, from, to){ return x.split(esc(from)).join(esc(to)).split(from).join(esc(to)); }
...
async function fill(d){
  ...
  // ── ADDENDUM NUMBER ───────────────────────────────────────
  // Template: "ADDENDUM №" THEN "14" THEN " TO CONTRACT "
  x = x.replace(
    /(ADDENDUM \u2116<\/w:t><\/w:r><w:r[^>]*><w:rPr[\s\S]*?<\/w:rPr><w:t[^>]*>)14(<\/w:t>)/,
    '$1'+esc(num)+'$2'
  );
  // Russian title "ПРИЛОЖЕНИЕ №14 К КОНТРАКТУ"
  x = x.replace(/ПРИЛОЖЕНИЕ \u211614 К КОНТРАКТУ /g, 'ПРИЛОЖЕНИЕ \u2116'+esc(num)+' К КОНТРАКТУ ');

  // ── CONTRACT REFERENCE ────────────────────────────────────
  // Template splits: "KAR-MOJ-USD " | "01/2025" | "dated 02" | ".07.2025"
  // RU splits:       "KAR-MOJ-USD " | "01/2025  от 02" | ".07.2025"
  const cparts = contract.match(/^([^\s]+)\s+(.+)/) || [null,'KAR-MOJ-USD','01/2025'];
  const ccode = cparts[1], cver = cparts[2];
  x = rep(x, 'KAR-MOJ-USD ', ccode+' ');
  x = rep(x, '01/2025', cver);
  x = rep(x, 'dated 02', 'dated '+dc.slice(0,2));
  x = rep(x, '.07.2025', dc.slice(2));
  // Fix RU: "dd" → "от"
  x = x.replace(new RegExp(esc(cver)+'  dd ','g'), esc(cver)+'  от ');

  // ── DATE ─────────────────────────────────────────────────
  // Template splits date into: ": 2" | "3" | "." | "03" | ".20" | "26"
  const dd=date.slice(0,2), mm=date.slice(3,5), yy=date.slice(6,10);
  // EN date (": 2" then "3" then "." then "03" then ".20" then "26")
  x = x.replace(/>: 2<\/w:t>(<\/w:r><w:r[^>]*><w:rPr[\s\S]*?<\/w:rPr><w:t[^>]*>)3(<\/w:t><\/w:r><w:r[^>]*><w:rPr[\s\S]*?<\/w:rPr><w:t[^>]*>)\.(<\/w:t><\/w:r><w:r[^>]*><w:rPr[\s\S]*?<\/w:rPr><w:t[^>]*>)03(<\/w:t><\/w:r><w:r[^>]*><w:rPr[\s\S]*?<\/w:rPr><w:t[^>]*>)\.20(<\/w:t><\/w:r><w:r[^>]*><w:rPr[\s\S]*?<\/w:rPr><w:t[^>]*>)26(<\/w:t>)/,
    '>: '+dd[0]+'</w:t>$1'+dd[1]+'$2.$3'+mm+'$4.'+yy.slice(0,2)+'$5'+yy.slice(2)+'$6');
  // RU date (": " | "2" | "3" | "." | "03" | ".20" | "26")
  x = x.replace(/(<w:t[^>]*>: <\/w:t><\/w:r><w:r[^>]*><w:rPr[\s\S]*?<\/w:rPr><w:t[^>]*>)2(<\/w:t><\/w:r><w:r[^>]*><w:rPr[\s\S]*?<\/w:rPr><w:t[^>]*>)3(<\/w:t><\/w:r><w:r[^>]*><w:rPr[\s\S]*?<\/w:rPr><w:t[^>]*>)\.(<\/w:t><\/w:r><w:r[^>]*><w:rPr[\s\S]*?<\/w:rPr><w:t[^>]*>)03(<\/w:t><\/w:r><w:r[^>]*><w:rPr[\s\S]*?<\/w:rPr><w:t[^>]*>)\.20(<\/w:t><\/w:r><w:r[^>]*><w:rPr[\s\S]*?<\/w:rPr><w:t[^>]*>)26(<\/w:t>)/,
    '$1'+dd[0]+'$2'+dd[1]+'$3.$4'+mm+'$5.'+yy.slice(0,2)+'$6'+yy.slice(2)+'$7');
  ...
  // ── QUANTITY ──────────────────────────────────────────────
  // Template: "25 " | "tons net" | "t" | ","  and  "25" | "тонн" | "нетто"
  const qty = String(d.qty||'25').split(' ')[0];
  x = x.replace(/>25 <\/w:t>(<\/w:r><w:r[^>]*><w:rPr[\s\S]*?<\/w:rPr><w:t[^>]*>)tons net(<\/w:t>)/,
    '>'+esc(qty)+' </w:t>$1tons net</w:t>');
  x = x.replace(/>25<\/w:t>(<\/w:r><w:r[^>]*><w:rPr[\s\S]*?<\/w:rPr><w:t[^>]*>)тонн(<\/w:t>)/,
    '>'+esc(qty)+'</w:t>$1тонн</w:t>');

  // ── PRICE ─────────────────────────────────────────────────
  // Template EN: "1" | "140" | " USD/MT"
  // Template RU: "1140 " | "USD" | "/т"
  const priceEN = (d.price_en||'1140 USD/MT');
  const priceRU = (d.price_ru||'1140 USD/т');
  // Replace the three EN price runs
  x = x.replace(/>1<\/w:t>(<\/w:r><w:r[^>]*><w:rPr[\s\S]*?<\/w:rPr><w:t[^>]*>)140(<\/w:t><\/w:r><w:r[^>]*><w:rPr[\s\S]*?<\/w:rPr><w:t xml:space="preserve"> USD\/MT<\/w:t>)/,
    '>'+esc(priceEN.replace(/ per ton.*$/,'').replace(/[A-Z]+\s*/,''))+'</w:t>$1$2'.replace('1140',esc(priceEN)));
  // Simpler approach for price - replace whole content
  x = rep(x, '1140 USD/MT', priceEN);
  x = rep(x, 'Saint-Petersburg, Russia', d.incoterm||'Saint-Petersburg, Russia');
  x = rep(x, '1140 ', priceRU.replace('/т','').replace(' USD','').trim()+' ');
  x = rep(x, '/т', '/т');
  x = rep(x, 'Санкт-Петербург, Россия', d.incoterm_ru||d.incoterm||'');

  // ── TOTAL VALUE ───────────────────────────────────────────
  // Template EN: "28500" | "USD  " (separate runs)
  // Template RU: "2850" | "0" | " " | "USD"
  const totalNum = String(d.total_num||'28500');
  const cur = d.currency||'USD';
  x = x.replace(/>28500<\/w:t>(<\/w:r><w:r[^>]*><w:rPr[\s\S]*?<\/w:rPr><w:t xml:space="preserve">)USD  (<\/w:t>)/,
    '>'+esc(totalNum)+'</w:t>$1'+esc(cur)+'  $2');
  // RU total "2850" | "0" | " " | "USD"
  x = x.replace(/>2850<\/w:t>(<\/w:r><w:r[^>]*><w:rPr[\s\S]*?<\/w:rPr><w:t[^>]*>)0(<\/w:t><\/w:r><w:r[^>]*><w:rPr[\s\S]*?<\/w:rPr><w:t xml:space="preserve"> <\/w:t><\/w:r><w:r[^>]*><w:rPr[\s\S]*?<\/w:rPr><w:t[^>]*>)USD(<\/w:t>)/,
    '>'+esc(totalNum.slice(0,-1))+'</w:t>$1'+esc(totalNum.slice(-1))+'</w:t>$2'+esc(cur)+'</w:t>');
  ...
  x = rep(x, 'CFR ', '');
  x = rep(x, 'Санкт-Петербург, Россия', d.delivery_ru_2||'');

  // ── SIGNATURES ────────────────────────────────────────────
  // Template: "Dinara" | "Apiyeva" (separate runs)
